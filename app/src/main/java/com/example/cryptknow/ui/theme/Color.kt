package com.example.cryptknow.ui.theme

import androidx.compose.ui.graphics.Color

val Purple200 = Color(0xFFBB86FC)
val Purple500 = Color(0xFF6200EE)
val Purple700 = Color(0xFF3700B3)
val Teal200 = Color(0xFF03DAC5)
val primaryLight = Color(0xFF8E8E8E)
val Grey200 = Color(0xFF8E8E8E)
val primary = Color(0xFF616161)
val Grey500 = Color(0xFF616161)
val primaryDark = Color(0xFF373737)
val Grey700 = Color(0xFF373737)
val secondaryLight = Color(0xFFEFEFEF)
val LightGrey200 = Color(0xFFEFEFEF)
val secondary = Color(0xFFBDBDBD)
val LightGrey500 = Color(0xFFBDBDBD)
val secondaryDark = Color(0xFF8D8D8D)
val LightGrey700 = Color(0xFF8D8D8D)
val YellowSecLight = Color(0xFFffff6b)
val YellowSec = Color(0xFFFDD835)
val YellowSecDark = Color(0xFFc6a700)
val LightBlue = Color(0xFF4e516d)
val NormalBlue = Color(0xFF252942)
val DarkBlue = Color(0xFF00001c)


